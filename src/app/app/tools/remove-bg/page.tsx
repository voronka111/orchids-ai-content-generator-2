...
function RemoveBgToolContent() {
    const { language } = useLanguage();
    const searchParams = useSearchParams();
    const router = useRouter();

    const EXAMPLE_IMAGE =
        'https://slelguoygbfzlpylpxfs.supabase.co/storage/v1/render/image/public/project-uploads/010102a2-9a78-497c-a40c-16883620b037/removebg-1768346852348.png?width=8000&height=8000&resize=contain';
    ...
    useEffect(() => {
        ...
        if (generation.status === 'success' && generation.result_assets?.[0]?.url) {
            ...
            toast.success(
                language === 'ru' ? 'Фон успешно удален!' : 'Background successfully removed!'
            );
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const genId = await removeBackground({ image: imageUrl });
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-xs font-bold">
                                <span className="text-white/40">
                                    {language === 'ru' ? 'Тип фона' : 'Background'}
                                </span>
                                <span>{language === 'ru' ? 'Прозрачный' : 'Transparent'}</span>
                            </div>
                            <div className="flex justify-between text-xs font-bold">
                                <span className="text-white/40">
                                    {language === 'ru' ? 'Края' : 'Edges'}
                                </span>
