...
function PromptBar({
    ...
    const { imageModels, videoModels, audioModels, fetchModels } = useModelsStore();
    ...
    const getModel = () => {
        if (type === 'image') return imageModels[0]?.name || 'FLUX Pro';
        if (type === 'video') return videoModels[0]?.name || 'Kling';
        return audioModels[0]?.name || 'Suno';
    };
    ...
    const getAspectRatioIcon = (id: string, className?: string) => {
        switch (id) {
            case '1:1':
                return (
                    <div className={`w-3 h-3 border border-current rounded-none ${className}`} />
                );
            case '16:9':
                return (
                    <div className={`w-4 h-2.5 border border-current rounded-none ${className}`} />
                );
            case '9:16':
                return (
                    <div className={`w-2.5 h-4 border border-current rounded-none ${className}`} />
                );
            case '4:3':
                return (
                    <div className={`w-3.5 h-3 border border-current rounded-none ${className}`} />
                );
            case '3:2':
                return (
                    <div
                        className={`w-3.5 h-2.5 border border-current rounded-none ${className}`}
                    />
                );
            default:
                return (
    ...
    return (
        <div
            className={`w-full max-w-2xl mx-auto ${
                sticky ? 'fixed bottom-4 left-0 right-0 z-40 px-4 sm:px-6 pointer-events-none' : ''
            }`}
        >
            <div
...
function AppCard({
    ...
    return (
        <Link href={href}>
            <motion.div className="relative h-40 sm:h-52 rounded-2xl overflow-hidden group cursor-pointer">
                <img
                    src={image}
                    alt={title}
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
