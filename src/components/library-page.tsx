...
import { mockGenerations, Generation } from "@/lib/mock-data";
...
type CategoryType = "all" | "image" | "video" | "audio" | "face-swap" | "stylist" | "relight" | "favorites";
...
export function LibraryPage() {
  ...
  const filteredGenerations = useMemo(() => {
    return generations.filter((g) => {
      ...
      if (activeCategory === "favorites") return g.liked;
      ...
      // Mocking app categories for now as they aren't in the mock data
      return false;
    });
  ...
  const handleAddFolder = () => {
    ...
    const name = language === "ru" ? `Новая папка ${folders.length + 1}` : `New Folder ${folders.length + 1}`;
    setFolders([...folders, { id: Date.now().toString(), name, count: 0 }]);
  };

  const categories = [
    { id: "all", label: language === "ru" ? "Библиотека" : "Library", icon: Folder, count: generations.length },
    { id: "favorites", label: language === "ru" ? "Избранное" : "Favorites", icon: Heart, count: generations.filter(g => g.liked).length },
  ];

  const tools = [
    { id: "image", label: language === "ru" ? "Изображения" : "Images", icon: ImageIcon, count: generations.filter(g => g.type === "image").length },
    { id: "video", label: language === "ru" ? "Видео" : "Video", icon: Video, count: generations.filter(g => g.type === "video").length },
    { id: "audio", label: language === "ru" ? "Аудио" : "Audio", icon: Music, count: generations.filter(g => g.type === "audio").length },
  ];

  const apps = [
    { id: "face-swap", label: language === "ru" ? "Замена лица" : "Face Swap", icon: Smile, count: 0 },
    { id: "stylist", label: language === "ru" ? "Стилист" : "Stylist", icon: Palette, count: 0 },
              ...
              {categories.map(cat => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all ${
                    activeCategory === cat.id ? "bg-white/10 text-white" : "text-white/60 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <cat.icon className={`w-4 h-4 ${activeCategory === cat.id ? "text-[#6F00FF]" : ""}`} />
                    <span className="text-sm font-medium">{cat.label}</span>
                  </div>
                  <span className="text-[10px] font-bold opacity-40">{cat.count}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1">
              {tools.map(tool => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all ${
                    activeCategory === tool.id ? "bg-white/10 text-white" : "text-white/60 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <tool.icon className={`w-4 h-4 ${activeCategory === tool.id ? "text-[#6F00FF]" : ""}`} />
                    <span className="text-sm font-medium">{tool.label}</span>
                  </div>
                  <span className="text-[10px] font-bold opacity-40">{tool.count}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1">
              {apps.map(app => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all ${
                    activeCategory === app.id ? "bg-white/10 text-white" : "text-white/60 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <app.icon className={`w-4 h-4 ${activeCategory === app.id ? "text-[#6F00FF]" : ""}`} />
                    <span className="text-sm font-medium">{app.label}</span>
                  </div>
                  <span className="text-[10px] font-bold opacity-40">{app.count}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <main className="flex-1 space-y-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <h1 className="text-3xl font-black uppercase tracking-tight">
              {activeCategory === "all" ? (language === "ru" ? "Библиотека" : "Library") : 
               activeCategory === "favorites" ? (language === "ru" ? "Избранное" : "Favorites") :
          ...
          {groupedGenerations.length > 0 ? (
                      ...
                      >
                        {gen.type === "audio" ? (
                          <div className="w-full h-full bg-gradient-to-br from-white/5 to-white/[0.02] flex flex-col items-center justify-center p-6 text-center border border-white/5">
                            ...
                            <p className="text-sm font-medium line-clamp-2 mb-2 px-4">{gen.prompt}</p>
                            <span className="text-[10px] font-bold text-white/20 uppercase tracking-widest">{gen.model}</span>
                          </div>
                        ) : (
                          <>
                            <img
                              src={gen.thumbnail || gen.url}
                              alt={gen.prompt}
                              className="w-full h-full object-cover"
                            />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-end">
                              <p className="text-xs font-medium line-clamp-2">{gen.prompt}</p>
                            </div>
                            {gen.type === "video" && (
                              <div className="absolute top-4 left-4 w-8 h-8 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center">
                        ...
                        <div className={`absolute top-4 right-4 w-6 h-6 rounded-full border-2 transition-all flex items-center justify-center ${
                          ...
                          {selectedIds.includes(gen.id) && <Check className="w-3.5 h-3.5 text-white" />}
                        </div>
                      </motion.div>
                    ))}
                  </div>
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center">
              ...
              <p className="text-white/40">
                {language === "ru" ? "Сделайте, чтобы было." : "Create something to see it here."}
              </p>
            </div>
          )}
        </main>
